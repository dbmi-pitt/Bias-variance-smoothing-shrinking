<p>We borrow some function names from R to indicate the corresponding probability functions <em>dmultinom</em> (for the multinomial distribution) , and a density function <em>ddirichlet</em> (for the Dirichlet distribution).</p>

<p>We let \(Dir\) be the Dirichlet distribution normalizer constant, defined by 
\[\begin{array}{c}
...
\end{array}\]
where \(Gamma\) is the standard gamma function, with the convenient property \(Gamma(n) = (n-1)!\).</p>
...
\end{array}\]
For the original data, this is</p>
